...
PostgreSQL.prototype.executeSQL = function(sql, params, options, callback) {
  ...
  if (transaction && transaction.connector === this) {
    if (!transaction.connection || !transaction.connection.postgresConnection) {
      return process.nextTick(function() {
        callback(new Error(g.f('Connection does not exist')));
      });
    }
    if (transaction.connection.id !== transaction.connection.postgresConnection.txId) {
      return process.nextTick(function() {
    ...
    executeWithConnection(transaction.connection.postgresConnection, null);
  } else {
