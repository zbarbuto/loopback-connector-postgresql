...
function mixinTransaction(PostgreSQL) {
   ...
   * @param cb
   */
  PostgreSQL.prototype.commit = function(transaction, cb) {
    debug('Commit a transaction');
    var self = this;
    transaction.postgresConnection.query('COMMIT', function(err) {
      self.releaseConnection(transaction.postgresConnection, err);
      cb(err);
    });
  };
   ...
   * @param cb
   */
  PostgreSQL.prototype.rollback = function(transaction, cb) {
    debug('Rollback a transaction');
    var self = this;
    transaction.postgresConnection.query('ROLLBACK', function(err) {
